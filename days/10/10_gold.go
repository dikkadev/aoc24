...
func Solve(input *input.Input, log *slog.Logger) int {
	...
	// for i := 0; i < len(tXs); i++ {
	// 	for j := 0; j < len(pXs); j++ {
	// 		if t.PathDFS(tXs[i], tYs[i], pXs[j], pYs[j]) {
	...
	for i := 0; i < len(tXs); i++ {
		for j := 0; j < len(pXs); j++ {
			if n := t.PathBFS(tXs[i], tYs[i], pXs[j], pYs[j]); n > 0 {
				slog.Debug("path found", "from", [2]int{tXs[i], tYs[i]}, "to", [2]int{pXs[j], pYs[j]}, "n", n)
				result += n
			}
		}
	}
...
func (t *Topo) PathDFS(startX, startY, goalX, goalY int) bool {
	//dfs
	stack := make([][2]int, 0)
...
// returns the amount of different paths there are
func (t *Topo) PathBFS(startX, startY, goalX, goalY int) int {
	pathCount := 0
	...
	for len(queue) > 0 {
		...
		visitedForMarkedX = append(visitedForMarkedX, current[0])
		visitedForMarkedY = append(visitedForMarkedY, current[1])
		// slog.Debug("visiting", "current", current, "goal", [2]int{goalX, goalY}, "queue", queue, "topo", t.StringMarked2(visitedForMarkedX, visitedForMarkedY, []int{goalX}, []int{goalY}))
		visited[current] = struct{}{}
		if current[0] == goalX && current[1] == goalY {
			pathCount++
			continue
