...
func Solve(input *input.Input, log *slog.Logger) int {
	...
	for _, u := range updates {
		if !u.IsCorrect(rules) {
			...
			slog.Debug("", "incorrect", u)
			corrected := u.Correct(rules)
			slog.Debug("", "corrected", corrected)
			result += corrected.Middle()
		}
	}
...
func (r Rule) Holds(series Update) (bool, int, int) {
	...
	return idxLeft < idxRight, idxLeft, idxRight
}
...
func (u *Update) Correct(rules []Rule) *Update {
	swap := func(u *Update, i, j int) {
		(*u)[i], (*u)[j] = (*u)[j], (*u)[i]
	}

	for !u.IsCorrect(rules) {
		for _, r := range rules {
			holds, left, right := r.Holds(*u)
			if !holds {
				swap(u, left, right)
			}
		}
